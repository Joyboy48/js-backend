      ...
      <div className="flex gap-4 mb-10">
        ...
        <form onSubmit={handleAddComment} className="flex-1 flex flex-col items-end gap-2">
          <input
            ...
            className="w-full bg-transparent border-b border-white/20 pb-2 focus:border-primary outline-none transition-colors text-sm font-medium"
          />
          {newComment && (
            <div className="flex gap-2">
               <button type="button" onClick={() => setNewComment("")} className="px-4 py-2 text-sm font-medium hover:bg-white/10 rounded-full transition-colors">Cancel</button>
            ...
            <div key={comment._id} className="flex gap-4 group">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between">
                   <div>
                       <span className="font-bold text-sm">{comment.owner?.fullName}</span>
                       <span className="text-gray-500 text-xs ml-2">{new Date(comment.createdAt).toLocaleDateString()}</span>
                   </div>
                   <div className="relative">
                      <button onClick={() => setActiveMenu(activeMenu === comment._id ? null : comment._id)} className="text-white/0 group-hover:text-white/50 hover:!text-white transition-colors p-1">
                         <MoreVertical size={16} />
                      </button>
                      {activeMenu === comment._id && (
                          <div className="absolute right-0 top-6 w-32 bg-[#121214] border border-white/10 rounded-xl shadow-2xl py-1 z-10">
                              <button onClick={() => handleDelete(comment._id)} className="w-full px-4 py-2 text-left text-sm hover:bg-white/5 flex items-center gap-2 text-red-400">
